...
from nba.models import Player, Team, Year
...
class ModelsTestCase(unittest.TestCase):   
    ...
    def test_Player1(self):
        lal = Team.objects.get(name="Lakers")
        ...
        self.assertEqual(kobe.hightlights, "http://youtube.com")
        ...
        self.assertEqual(kobe.finalsmvp_years, "2008")
    ...
    def test_Year1(self):
        lal = Team.objects.get(name="Lakers")
        kobe = Player.objects.get(name="Kobe Bryant")
    ...
    def test_Player2(self):
        ...
        self.assertEqual(tim.current_team, 'none')
   ...
   def test_Year2(self):
        ...
        self.assertEqual(y2007.reg_mvp, 'none')
        self.assertEqual(y2007.finals_mvp, 'none')
        self.assertEqual(y2007.champion, 'none')

    #-----
    # test created (dummy) database
    # ----

    # Players
    def test_fakeDB1(self):
        kobe = Player.objects.get(name="Kobe Bryant")
        tim = Player.objects.get(name="Tim Duncan")

        # test uniques
        if(kobe.nickname == tim.nickname) :
            assert False
        if(kobe.twitter == tim.twitter) :
            assert False
        if(kobe.photo == tim.photo) :
            assert False
        if(kobe.highlights == tim.highlights) :
            assert False
        
        
    # Teams
    def test_fakeDB2(self):
        lal = Team.objects.get(name="Lakers")
        sa = Team.objects.get(name="Spurs")
        
        # test uniques
        if(lal.location == sa.location):
            assert False
        if(lal.coach == sa.coach):
            assert False
        if(lal.gm == sa.gm):
            assert False
        if(lal.twitter == sa.twitter):
            assert False
        if(lal.logo = sa.logo):
            assert False
    ...
    # Years
    def test_fakeDB3(self):
        y2006 = Year.objects.get(year="2006")
        y2007 = Year.objects.get(year="2007")

        # test uniques
        if(y2006.finals_logo == y2007.finals_logo):
            assert False
        if(y2006.highlights == y2007.highlights):
            assert False
